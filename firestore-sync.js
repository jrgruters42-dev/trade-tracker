(function (root) {
    ...
    const COLLECTIONS = [
        ...
        { key: 'closedTrades', name: 'closedTrades', prefix: 'trade', idField: 'id', shape: 'array' },
        ...
        if (definition.shape === 'indexedObject') {
            if (Array.isArray(source)) {
                return source.filter(value => value && typeof value === 'object').map((value, index) => {
                    let slotStr = (value._slot !== undefined && value._slot !== null && value._slot !== '' && String(value._slot) !== 'undefined')
                        ? String(value._slot)
                        : (value._syncId ? String(value._syncId).replace(new RegExp('^' + definition.prefix + '-'), '') : String(index + 1));
                    if (slotStr === '0') slotStr = '1';
                    return Object.assign({}, value, {
                        _slot: slotStr,
                        _syncId: `${definition.prefix}-${slotStr}`,
                        _syncOrder: Number.isFinite(value._syncOrder) ? value._syncOrder : index
                    });
                });
            }
            return Object.entries(source || {})
                .filter(([slot, value]) => slot !== 'undefined' && value && typeof value === 'object')
                .map(([slot, value], index) => {
                    let slotStr = (value._slot !== undefined && value._slot !== null && value._slot !== '' && String(value._slot) !== 'undefined')
                        ? String(value._slot)
                        : (value._syncId ? String(value._syncId).replace(new RegExp('^' + definition.prefix + '-'), '') : String(slot).replace(new RegExp('^' + definition.prefix + '-'), ''));
                    if (slotStr === '0') slotStr = '1';
                    return Object.assign({}, value, {
                        _slot: slotStr,
                        _syncId: `${definition.prefix}-${slotStr}`,
    ...
    class JournalFirestoreStore {
        ...
        async load() {
            ...
            COLLECTIONS.forEach((definition, index) => {
                const items = collectionSnapshots[index].docs.map(document => {
                    ...
                    if (definition.shape === 'indexedObject') {
                        const docSlot = document.id.startsWith(definition.prefix + '-')
                            ? document.id.slice(definition.prefix.length + 1)
                            : document.id;
                        payload._slot = String(payload._slot || docSlot);
                    }
                    return payload;
        ...
        async save(localData, baseCheckpoint, options = {}) {
            const base = baseCheckpoint || this.getCheckpoint();
            if (root.TradeSyncSafety) {
                const safetyCheck = root.TradeSyncSafety.validatePayloadSafety(localData, base, options);
                if (!safetyCheck.safe) {
                    if (options && options.deletionToken) {
                        root.TradeSyncSafety.invalidateToken(options.deletionToken);
                    }
            ...
            if (operations.length === 0) {
                if (options && options.deletionToken) {
                    root.TradeSyncSafety.consumeToken(options.deletionToken);
                }
